...
# MAGIC ## Auto Loader
# MAGIC
# MAGIC will not active yet unless until we do a display or write stream
# MAGIC
...
(spark.readStream
    ...
    .option("cloudFiles.schemaLocation", "dbfs:/mnt/demo/checkpoints/orders_raw") #where that allow PYSpark store schema information, enabling schema inference and evolution.
...
# MAGIC
# MAGIC current_timestamp, input_file_name. Such information is useful for troubleshooting errors if corrupted data is encountered.
...
# MAGIC %md
# MAGIC This select statement below trigger spark.readStream atatement above active
...
# MAGIC
# MAGIC Passing this enriched data back to PySpark API to process an incremental write to a Delta Lake
...
# MAGIC
# MAGIC This is a PySpark API statement, but we can achieve the same result using Spark SQL.
